...
// ✅ dynamic wallet columns
export function makeWalletColumns(opts: { canManageWallets: boolean }): ColumnDef<VendorWalletRow>[] {
  const base: ColumnDef<VendorWalletRow>[] = [
    {
      header: "Vendor",
    ...
    },
    {
      ...
      accessorKey: "balance",
      cell: ({ row }) => (
        <span className="font-mono font-bold text-zinc-900">{row.original.balance}</span>
      ),
    },
    {
      ...
      accessorKey: "pending",
      cell: ({ row }) => (
        <span className="font-mono text-zinc-500 text-xs">{row.original.pending}</span>
      ),
    },
    {
      header: "Last Payout",
      ...
      cell: ({ row }) => (
        <span className="font-mono text-xs text-zinc-500">{row.original.lastPayout}</span>
      ),
    },
    {
      header: "Status",
  ...
  if (!opts.canManageWallets) return base;
    ...
    ...base,
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-2">
          <ToggleFreezeButton vendorId={row.original.vendorId} currentStatus={row.original.status} />
          <PayoutModal
...
// transaction columns stay the same (no sensitive actions)
export const transactionColumns: ColumnDef<FinanceTransactionRow>[] = [
        ...
        <div className="flex items-center gap-2">
          <div
            className={`h-6 w-6 rounded-full flex items-center justify-center ${
              isIn ? "bg-emerald-100 text-emerald-600" : "bg-zinc-100 text-zinc-600"
            }`}
          >
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const isIn = row.original.amount.includes("+");
      return (
        <span className={`font-mono font-bold ${isIn ? "text-emerald-600" : "text-zinc-900"}`}>
          {row.original.amount}
        </span>
      );
    },
  ...
  },
  {
    ...
    accessorKey: "date",
    cell: ({ row }) => (
      <span className="font-mono text-xs text-zinc-400">{row.original.date}</span>
    ),
  },
  {
    header: "Status",
