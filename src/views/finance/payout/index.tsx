...
import { useWithdrawals, useWithdrawalStats } from "@/src/features/withdrawals/hooks";
      ...
      <main className="p-6 max-w-400 mx-auto space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatCard
            ...
            value={statsQ.isLoading ? "—" : String(statsQ.data?.pendingRequests ?? 0)}
            icon={AlertCircle}
            variant="rose"
          />
          <StatCard
            label="Processed Today"
            value={statsQ.isLoading ? "—" : String(statsQ.data?.processedToday ?? 0)}
            icon={CheckCircle2}
            variant="cyan"
          />
          <StatCard
            ...
            value={
              statsQ.isLoading
                ? "—"
                : money(statsQ.data?.totalDisbursed ?? 0, statsQ.data?.currency ?? currency)
            }
            icon={History}
            variant="emerald"
          />
        ...
        >
          <div className="flex flex-col md:flex-row md:items-end justify-between border-b border-zinc-200 gap-4 md:gap-0 pb-2 md:pb-0">
            <FilterTabs
              tabs={[
                { value: "pending", label: "Queue", count: pending.length, variant: "rose" },
                { value: "history", label: "History", count: history.length, variant: "indigo" },
              ]}
            />
          ...
          <TabsContent value="pending" className="m-0">
            {pendingQ.isLoading ? (
              ...
              <div className="p-6 text-sm text-rose-600">Failed to load pending requests.</div>
            ) : pending.length > 0 ? (
          ...
          <TabsContent value="history" className="m-0">
            {historyQ.isLoading ? (
              ...
              <div className="p-6 text-sm text-rose-600">Failed to load payout history.</div>
            ) : history.length > 0 ? (
