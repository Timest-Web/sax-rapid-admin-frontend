...
export function makeVendorColumns(opts: { canViewDetails: boolean }): ColumnDef<VendorProfile>[] {
  return [
    {
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 bg-white border-zinc-200">
              ...
              {opts.canViewDetails ? (
                ...
                <DropdownMenuItem
                  disabled
                  className="text-xs text-zinc-400 cursor-not-allowed flex items-center w-full"
                >
                  <Eye className="mr-2 h-3.5 w-3.5 text-zinc-300" /> View Details (No access)
                </DropdownMenuItem>
              )}
            </DropdownMenuContent>
          </DropdownMenu>
